...
. A new session request arrives to ESG.
. All requests during startup are marked as `pending`. Before proceeding to next step Selenoid waits for required port to be open. This is done by sending HEAD requests to the port.
