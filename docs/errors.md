...
3. `service startup timed out` - task was recreated several times because of a failure. In the end couldn't wait until task is running and healthy.
...
1. `session timed out or not found` - session didn't exist or cache was already flushed.
...
1. `task timed out or not found` - task didn't exist or cache was already flushed.
