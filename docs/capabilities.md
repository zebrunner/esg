...
* `videoScreenSize` - Default: value from screenResolution. Value type: string. Determines recording screen resolutin. Cannot be higher than actual screen resolution. Max total pixels: 3_000_000 (example: 1920x1080=2_073_600).
...
> Max limitation for cpu/memory is configurable uniquely for every ESG server. Max values by default: cpu: 16384, memory: 28675.
...
* `mitmMemory` (allias: `MitmMemory`, `mitmmemory`) - Min: 512. Default: 512. Value type: intefer/string. Memory (RAM) limitation for mitm container measured in MiB.
