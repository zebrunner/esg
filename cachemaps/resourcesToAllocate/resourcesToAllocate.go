...
func GetEntitiesOfCapacityProvider(capacityProvider string) ([]*ResourcesToAllocate, error) {
	...
	iter := config.REDIS_RESOURCES_CLIENT.GetConnection().Scan(context.Background(), 0, fmt.Sprintf("%s*", capacityProvider), 50).Iterator()
	...
	for uuid := range keysSet {
		data, err := config.REDIS_RESOURCES_CLIENT.GetConnection().Get(context.Background(), uuid).Result()
		if err != nil {
			if err == redis.Nil {
				continue
			}
